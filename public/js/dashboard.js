      ...
      const mainRow = `<tr class="${rowCls}">
        <td>
          ...
          <div style="display:flex;gap:.3rem;margin-top:.35rem;flex-wrap:wrap">
            <button class="icopy" style="font-size:.64rem;padding:.18rem .5rem" onclick="Notes.open('${token}')">📝 ${(t.notes||[]).length > 0 ? (t.notes.length)+' note'+(t.notes.length!==1?'s':'') : 'Add note'}</button>
            ${t.approved ? `<button class="icopy" style="font-size:.64rem;padding:.18rem .5rem;color:#7c3aed" onclick="RenewalModal.open('${token}')">🔁 Renew</button>` : ''}
          </div>
        </td>
