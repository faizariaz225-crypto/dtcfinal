...
if (!fs.existsSync(LANDING_FILE))   fs.writeFileSync(LANDING_FILE, JSON.stringify({  "hero": {    "badge": "AI-Powered Academic Tools",    "headline": "Supercharge Your Academic Journey",    "subheadline": "DTC — Digital Tools Corner provides premium AI subscriptions and academic software to students, researchers, and educators worldwide.",    "cta": "Explore Tools"  },  "tools": [    { "icon": "🤖", "name": "Claude Pro", "desc": "Anthropic's most capable AI assistant for research, writing, analysis and coding.", "tag": "AI Assistant" },    { "icon": "💬", "name": "ChatGPT Plus", "desc": "OpenAI's flagship model with GPT-4o, image generation and browsing capabilities.", "tag": "AI Assistant" },    { "icon": "📄", "name": "Turnitin", "desc": "Industry-leading plagiarism detection trusted by universities worldwide.", "tag": "Academic Integrity" },    { "icon": "📚", "name": "Grammarly Premium", "desc": "Advanced grammar, style and tone suggestions for polished academic writing.", "tag": "Writing" },    { "icon": "🔬", "name": "Consensus", "desc": "AI-powered academic search engine that finds evidence from peer-reviewed papers.", "tag": "Research" },    { "icon": "📊", "name": "Wolfram Alpha Pro", "desc": "Computational knowledge engine for mathematics, science and data analysis.", "tag": "Research" },    { "icon": "🧠", "name": "Perplexity Pro", "desc": "Real-time AI search with cited sources for fast, accurate research.", "tag": "AI Search" },    { "icon": "✍️", "name": "QuillBot Premium", "desc": "Advanced paraphrasing, summarising and citation tools for academic writing.", "tag": "Writing" }  ],  "stats": [    { "value": "500+", "label": "Active Students" },    { "value": "10+", "label": "AI Tools" },    { "value": "50+", "label": "Universities" },    { "value": "24/7", "label": "Support" }  ],  "features": [    { "icon": "⚡", "title": "Instant Activation", "desc": "Get access to your tools within hours of payment. No waiting, no delays." },    { "icon": "🔒", "title": "Secure & Trusted", "desc": "Your account details are handled with full privacy and security." },    { "icon": "💬", "title": "WeChat Support", "desc": "Dedicated support via WeChat throughout your entire subscription period." },    { "icon": "🔄", "title": "Easy Renewal", "desc": "Simple one-click renewal process to keep your tools running uninterrupted." }  ],  "contact": {    "wechat": "DTC_Support",    "email": "dtc@dtc1.shop",    "tagline": "Reach us anytime — we respond fast."  },  "footer": {    "tagline": "Empowering students and researchers with the world's best AI tools."  }}, null, 2));
...
const loadResellers   = () => JSON.parse(fs.readFileSync(RESELLERS_FILE,'utf8'));
...
const saveLanding     = d  => fs.writeFileSync(LANDING_FILE, JSON.stringify(d, null, 2));
...
function calcRevenue(tokens) {
  ...
  for (const t of Object.values(tokens)) {
    if (!t.approved || !t.price || t.deactivated) continue;
...
// ── Customer Notes ─────────────────────────────────────────────────────────────
app.post('/admin/notes/save', (req, res) => {
  const { adminKey, token, note } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  if (!tokens[token]) return res.status(404).json({ error: 'Not found.' });
  if (!tokens[token].notes) tokens[token].notes = [];
  const entry = { id: uuidv4(), text: note, createdAt: new Date().toISOString() };
  tokens[token].notes.push(entry);
  saveTokens(tokens);
  res.json({ success: true, note: entry });
});

app.post('/admin/notes/delete', (req, res) => {
  const { adminKey, token, noteId } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  if (!tokens[token]) return res.status(404).json({ error: 'Not found.' });
  tokens[token].notes = (tokens[token].notes || []).filter(n => n.id !== noteId);
  saveTokens(tokens);
  res.json({ success: true });
});

// ── Renewal link (pre-fill from existing token) ────────────────────────────────
app.post('/admin/generate-renewal', (req, res) => {
  const { adminKey, fromToken } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  const src = tokens[fromToken];
  if (!src) return res.status(404).json({ error: 'Source token not found.' });

  const { products } = loadProducts();
  const product = products.find(p => p.id === src.productId);
  if (!product) return res.status(400).json({ error: 'Product not found.' });

  const token = uuidv4();
  const expiresAt = new Date(Date.now() + LINK_EXPIRY_MS).toISOString();
  const instrId   = src.instructionSetId      || (product.type === 'chatgpt' ? 'chatgpt-plus' : 'default-claude');
  const postId    = src.postInstructionSetId  || instrId;
  ...
  tokens[token] = {
    ...
    price:               src.price,
    currency:            src.currency         || loadSettings().currency || 'USD',
    currencySymbol:      src.currencySymbol   || loadSettings().currencySymbol || '$',
    resellerId:          src.resellerId       || null,
    resellerName:        src.resellerName     || null,
    product:             src.product          || product.type,
    credentialsMode:     product.credentialsMode || false,
    loginDetails:        product.loginDetails    || '',
    instructionSetId:    instrId,
    postInstructionSetId:postId,
    createdAt:           new Date().toISOString(),
    expiresAt,
    durationDays:        src.durationDays     || getDurationDays(src.productId, src.packageType),
    renewedFromToken:    fromToken,
    used: false, approved: false, declined: false, deactivated: false,
  };
  saveTokens(tokens);
  const link = `${req.protocol}://${req.get('host')}/submit?token=${token}`;
  res.json({ link, token, expiresAt, price: src.price });
});

// ── Edit activated token ──────────────────────────────────────────────────────
app.post('/admin/edit-token', (req, res) => {
  const { adminKey, token, fields } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  if (!tokens[token]) return res.status(404).json({ error: 'Not found.' });
  const allowed = ['customerName','packageType','subscriptionDays','subscriptionExpiresAt','price','email','wechat'];
  for (const key of allowed) {
    if (fields[key] !== undefined) tokens[token][key] = fields[key];
  }
  tokens[token].editedAt = new Date().toISOString();
  saveTokens(tokens);
  res.json({ success: true });
});
...
app.post('/admin/send-custom-email', async (req, res) => {
  const { adminKey, to, subject, html, token } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  if (!to || !subject || !html) return res.status(400).json({ error: 'to, subject and html are required.' });
  res.json(await sendEmail({ to, subject, html, type: 'manual_custom', token: token || null }));
});
...
app.post('/reseller/login', (req, res) => {
  ...
  if (!resellerId || !password) return res.status(400).json({ error: 'Reseller ID and password required.' });
  const resellers = loadResellers();
  const r = resellers[resellerId];
  if (!r || r.password !== password) return res.status(401).json({ error: 'Invalid credentials.' });
  res.json({ ok: true, resellerId, name: r.name, commissionPct: r.commissionPct || 0 });
});

app.get('/reseller/data', (req, res) => {
  const { resellerId, password } = req.query;
  const resellers = loadResellers();
  const r = resellers[resellerId];
  if (!r || r.password !== password) return res.status(401).json({ error: 'Unauthorized' });
  const tokens = loadTokens();
  const settings = loadSettings();
  const myTokens = {};
  let total = 0; let activeCount = 0; let expiringSoon = 0;
  for (const [tok, t] of Object.entries(tokens)) {
    if (t.resellerId !== resellerId) continue;
    myTokens[tok] = t;
    if (t.approved && t.price && !t.deactivated) total += t.price;
    if (t.approved && !t.deactivated) {
      activeCount++;
      if (t.subscriptionExpiresAt) {
        const days = Math.ceil((new Date(t.subscriptionExpiresAt) - new Date()) / (1000*60*60*24));
        if (days >= 0 && days <= 30) expiringSoon++;
      }
    }
  }
  res.json({ tokens: myTokens, total, activeCount, expiringSoon, currencySymbol: settings.currencySymbol || '$', resellerName: r.name, commissionPct: r.commissionPct || 0 });
});

// ── Admin: Manage Reseller Accounts ──────────────────────────────────────────
app.get('/admin/reseller-accounts', (req, res) => {
  if (!isAdmin(req.query.adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const resellers = loadResellers();
  // Strip passwords from response
  const safe = Object.entries(resellers).map(([id, r]) => ({ id, name: r.name, commissionPct: r.commissionPct || 0, createdAt: r.createdAt }));
  res.json({ resellers: safe });
});

app.post('/admin/reseller-accounts/save', (req, res) => {
  const { adminKey, reseller } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  if (!reseller || !reseller.id || !reseller.name || !reseller.password)
    return res.status(400).json({ error: 'ID, name and password are required.' });
  const resellers = loadResellers();
  const existing = resellers[reseller.id] || {};
  const finalPassword = (reseller.password && reseller.password !== '__KEEP__') ? reseller.password : (existing.password || reseller.password);
  if (!finalPassword) return res.status(400).json({ error: 'Password is required.' });
  resellers[reseller.id] = { ...existing, name: reseller.name, password: finalPassword, commissionPct: parseFloat(reseller.commissionPct) || 0, createdAt: existing.createdAt || new Date().toISOString() };
  saveResellers(resellers);
  res.json({ success: true });
});

app.post('/admin/reseller-accounts/delete', (req, res) => {
  const { adminKey, resellerId } = req.body;
  if (!isAdmin(adminKey)) return res.status(401).json({ error: 'Unauthorized' });
  const resellers = loadResellers();
...
app.get('/landing-content', (req, res) => {
  res.json(loadLanding());
});
...
app.get('/customer/lookup', (req, res) => {
  ...
  const found = [];
  for (const [tok, t] of Object.entries(tokens)) {
    if (!t.email || t.email.toLowerCase() !== email.toLowerCase()) continue;
    if (!t.approved) continue;
    const days = t.subscriptionExpiresAt
      ? Math.ceil((new Date(t.subscriptionExpiresAt) - new Date()) / (1000*60*60*24))
      : null;
    found.push({
      token: tok,
      ...
      status:       t.deactivated ? 'deactivated' : (days !== null && days < 0) ? 'expired' : 'active',
      approvedAt:   t.approvedAt || null,
      subscriptionExpiresAt: t.subscriptionExpiresAt || null,
      durationDays: t.subscriptionDays || t.durationDays || 30,
      daysRemaining: days,
      // NO price exposed
    });
  }
  if (!found.length) return res.status(404).json({ error: 'No subscriptions found for this email.' });
  found.sort((a, b) => new Date(b.approvedAt || 0) - new Date(a.approvedAt || 0));
  res.json({ subscriptions: found, customerName: found[0].customerName });
});
